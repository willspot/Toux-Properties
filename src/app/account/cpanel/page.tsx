...
import { faList, faBuilding, faCouch, faImage, faUpload, faTimes } from '@fortawesome/free-solid-svg-icons';
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok && data.success) {
        setAlert({ type: 'success', message: `Property submitted successfully! Property ID: ${property_id}` });
      } else {
      ...
      <div className="w-full overflow-x-hidden bg-[#f7f3ef] min-h-screen flex flex-col items-center justify-start pt-2 sm:pt-4 px-4 sm:px-16">
        ...
        <section className="w-full max-w-6xl mx-auto py-8 sm:py-12">
          <div className="bg-white rounded-2xl shadow-lg p-6 sm:p-8">
            {/* Title */}
            <div className="mb-8">
              <h3 className="text-2xl sm:text-3xl font-semibold text-[#171717]">Submit Property</h3>
              <p className="text-red-900"><i>Note: Fill every details in every tab before clicking submit...</i></p>
            </div>
                      ...
                      <div className="border-2 border-dashed border-gray-300 rounded-lg p-6 text-center hover:border-[#b08a47] transition-colors">
                        {selectedGalleryImages.length > 0 ? (
                          <div className="space-y-3">
                            <div className="text-left">
                              <p className="font-medium text-gray-900 mb-2">Selected Images ({selectedGalleryImages.length})</p>
                            ...
                            <div className="flex items-center justify-center gap-4 pt-2">
                              <button
                                ...
                                className="bg-[#b08a47] text-white px-4 py-2 rounded-lg text-sm font-medium hover:bg-[#9a7a3d] transition-colors"
                              >
            ...
            <div className="mt-8 p-4 bg-gray-50 rounded-lg">
              <label className="text-sm font-medium text-gray-700">Agent Code: {process.env.NEXT_PUBLIC_AGENT_CODE}</label>
            </div>
          </div>
        </section>
